...
def render_set(
    model_path, name, iteration, views, gaussians, pipeline, background, near
):
    ...
    for idx, view in enumerate(
        tqdm(views, desc="Rendering progress", ascii=True, dynamic_ncols=True)
    ):
        mask_temp = (
            gaussians.get_xyz - view.camera_center.repeat(gaussians.get_xyz.shape[0], 1)
        ).norm(dim=1, keepdim=True) < near
    ...
    for idx, view in enumerate(
        ...
        depth = (render_pkg["depth"] - render_pkg["depth"].min()) / (
            render_pkg["depth"].max() - render_pkg["depth"].min()
        ) + 1 * (1 - render_pkg["alpha"])
        torchvision.utils.save_image(
            rendering, os.path.join(render_path, "{0:05d}".format(idx) + ".png")
        )
        torchvision.utils.save_image(
            1 - depth, os.path.join(render_path, "depth_{0:05d}".format(idx) + ".png")
        )

        depth_est = depth.squeeze().cpu().numpy()
        depth_est = visualize_cmap(
            depth_est,
            np.ones_like(depth_est),
            cm.get_cmap("turbo"),
            curve_fn=depth_curve_fn,
        ).copy()
        depth_est = torch.as_tensor(depth_est).permute(2, 0, 1)
        torchvision.utils.save_image(
            depth_est, os.path.join(render_path, "cdepth_{0:05d}".format(idx) + ".png")
        )
    ...
    os.system(
        ...
        + f"/%5d.png -q 2 "
        + model_path
        + "/out_{}.mp4 -y".format(model_path.split("/")[-1])
    )
    os.system(
        f"ffmpeg -i "
        + render_path
        + f"/depth_%5d.png -q 2 "
        + model_path
        + "/out_depth_{}.mp4 -y".format(model_path.split("/")[-1])
    )
    os.system(
        f"ffmpeg -i "
        + render_path
        + f"/cdepth_%5d.png -q 2 "
        + model_path
        + "/out_cdepth_{}.mp4 -y".format(model_path.split("/")[-1])
    )
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, near):
    with torch.no_grad():
        ...
        (model_params, _) = torch.load(
            os.path.join(dataset.model_path, f"chkpnt{loaded_iter}.pth")
        )
